...
import { Search, Filter, Atom, FlaskConical, Zap, Target, Clock, AlertCircle, Eye, Sparkles, ChevronDown, Plus, Minus, Flame, BookOpen, ChevronRight } from 'lucide-react';
...
function InorganicReactions() {
  ...
  useEffect(() => {
    setReactions(inorganicReactionsData.reactions);
    if (inorganicReactionsData.reactions.length > 0) {
      setSelectedReaction(inorganicReactionsData.reactions[0]);
    }
  }, []);
  ...
  // Professional color scheme for inorganic reaction types
  const typeColors = {
    combination: '#34d399',     // Green
    decomposition: '#f87171',   // Red
    displacement: '#60a5fa',    // Blue
    redox: '#c084fc',           // Purple
    'acid-base': '#fbbf24',     // Yellow
    all: '#818cf8'              // Indigo
  };

  const getTypeIcon = (type) => {
    const icons = {
      combination: <Plus size={16} />,
      decomposition: <Minus size={16} />,
      displacement: <Target size={16} />,
      redox: <Flame size={16} />,
      'acid-base': <Zap size={16} />
    };
      ...
      >
        <div style={{
          ...
          <div>
            ...
            >
              Interactive visualization of inorganic chemical mechanisms and ionic interactions
            </motion.p>
          </div>
        </div>
        ...
        >
          ...
          <div style={{ position: 'relative' }}>
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
            >
              <Filter size={18} color={typeColors[filter] || '#818cf8'} />
              <span>{filter === 'all' ? 'All Types' : filter === 'acid-base' ? 'Acid-Base' : filter.charAt(0).toUpperCase() + filter.slice(1)}</span>
              <ChevronDown 
                size={16} 
                style={{ 
                  marginLeft: 'auto',
            ...
            {isFilterOpen && (
              ...
              >
                ...
                {['combination', 'decomposition', 'displacement', 'redox', 'acid-base'].map((type, i) => (
                  <motion.button
                    key={type}
                  ...
                  >
                    {type === 'acid-base' ? 'Acid-Base' : type.charAt(0).toUpperCase() + type.slice(1)}
                  </motion.button>
                ))}
              </motion.div>
            )}
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{
                ...
                Explore detailed inorganic reaction mechanisms with interactive visualizations
              </p>
            </div>
          ...
          <div style={{
            ...
            {Object.entries(typeColors).filter(([type]) => type !== 'all').map(([type, color]) => (
              ...
              >
                ...
                <span>{type === 'acid-base' ? 'Acid-Base' : type.charAt(0).toUpperCase() + type.slice(1)}</span>
                <span style={{
                  background: `${color}30`,
                ...
                >
                  <div style={{
                    ...
                    <div style={{
                      ...
                      {reaction.type === 'acid-base' ? 'Acid-Base' : reaction.type.charAt(0).toUpperCase() + reaction.type.slice(1)}
                    </div>
                  </div>
                  ...
                  <div style={{
                    ...
                    <Clock size={14} />
                    <span>Complexity: {reaction.complexity || 'Medium'}</span>
                  </div>
                </motion.div>
              ))}
            </div>
              ...
              <div>
                ...
                {selectedReaction && (
                  <p style={{
                    ...
                    {selectedReaction.name} • {selectedReaction.type === 'acid-base' ? 'Acid-Base' : selectedReaction.type}
                  </p>
                )}
              </div>
              ...
              {selectedReaction && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    {selectedReaction.type === 'acid-base' ? 'Acid-Base' : selectedReaction.type.charAt(0).toUpperCase() + selectedReaction.type.slice(1)}
                  </div>
                </div>
              )}
            </div>
...
export default InorganicReactions;